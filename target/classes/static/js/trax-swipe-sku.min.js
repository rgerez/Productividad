function no_sol_action(element, csrfID) {
    var element_id = element.attr('element-id')
    var kpi_id = element_id.split('-');
...
$(document).on("swiperight", ".swipe-row .swipe-element", function (e) {
    ...
    var csrfID = $(this).attr('element-csrf');
    swipe_action(element_id, "R", type, csrfID);
...
function swipe_action(element_id, direction, type, csrfID) {
    ...
    if (direction === "L") {
        ...
        var no_solution = $("#no-solution-" + element_id).html();
    ...
    if (direction === "R") {
        ...
        $.ajax({
            ...
            success: function (data) {
                $(".swipe-row[element-id|='" + element_id + "']").attr('no-sol-id', "");
                $(".swipe-row[element-id|='" + element_id + "']").find('.swipe-indicator')
...
function sol_code_action(element) {
    ...
    var type = $(".swipe-row-inv[element-id|='" + element_id + "']").attr('element-type');
    if (code == "") {
        $(".swipe-row-inv[element-id|='" + element_id + "']").find('.swipe-indicator')
...
function no_sol_action_inv(element) {
    var element_id = element.attr('element-id');
    var kpi_id = element_id.split('-');
    var type = $(".swipe-row-inv[element-id|='" + element_id + "']").attr('element-type');
    $.ajax({
        ...
        success: function (data) {
            $('.ul-link-nosol').removeClass('selected');
            $(".swipe-row-inv[element-id|='" + element_id + "']").attr('no-sol-id', element.attr('nosolution-val'));
            element.addClass('selected');
            $(".swipe-row-inv[element-id|='" + element_id + "']").find('.swipe-indicator')
                .removeClass().addClass("swipe-indicator swipe-indicator0");
        }
...
$(document).on("swiperight", ".swipe-row-inv .swipe-element-inv", function (e) {
    $(this).css({
        transform: "translateX(+100px)"
    }).one("transitionend webkitTransitionEnd oTransitionEnd otransitionend MSTransitionEnd", function () {
        $(this).css({
    ...
    var element_id = $(this).attr('element-id');
    var type = $(this).attr('element-type');
    swipe_action_inv(element_id, "R", type);

});

$(document).on("swipeleft", ".swipe-row-inv .swipe-element-inv", function (e) {
    $(this).css({
        transform: "translateX(-100px)"
    }).one("transitionend webkitTransitionEnd oTransitionEnd otransitionend MSTransitionEnd", function () {
        $(this).css({
    ...
    swipe_action_inv(element_id, "L", type);
});



function swipe_action_inv(element_id, direction, type) {
    $('.causes').slideUp(function () {
        $(this).remove();
    });
    ...
    if (direction === "L") {
        var no_sol_id = $(".swipe-row-inv[element-id|='" + element_id + "']").attr('no-sol-id');
        var no_solution = $("#no-solution-" + element_id).html();
        $(".swipe-row-inv[element-id|='" + element_id + "']").after('<div class="causes" element-id="' + element_id + '">');
        $(".causes[element-id|='" + element_id + "']").html(no_solution).hide();
        $(".causes[element-id|='" + element_id + "']").slideDown();
        $(".ul-link-nosol").each(function (index) {
            if (no_sol_id == $(this).attr("nosolution-val")) {
    ...
    if (direction === "R") {
        ...
        $(".swipe-row-inv[element-id|='" + element_id + "']").after('<div class="positive" element-id="' + element_id + '">');
        ...
        var tmpCode = $(".swipe-row-inv[element-id|='" + element_id + "']").attr('sol_code');
